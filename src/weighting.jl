...
function LinearAlgebra.mul!(a::ProjectedField{G}, A::FarazmandWeight{T}) where {S, T, G<:ChannelGrid1D{S, T}}
...
function LinearAlgebra.dot(a::ProjectedField{G}, A::FarazmandWeight{T}, b::ProjectedField{G}) where {S, T, G<:ChannelGrid1D{S, T}}
    sum = zero(T)
