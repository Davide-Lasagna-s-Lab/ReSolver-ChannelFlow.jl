...
```@docs
NSEBase.ddy!(::NSEBase.FTField{G}, ::NSEBase.FTField{G}; adjoint=false) where {G <: ReSolverChannelFlow.AbstractChannelGrid}
NSEBase._inhomogeneous_laplacian!(::NSEBase.FTField{G}, ::NSEBase.FTField{G}; adjoint::Bool=false) where {G <: ReSolverChannelFlow.AbstractChannelGrid}
```
