...
# ============================================================
function make_methods()
    Dict(
        "Modal 1"   => CUDAExt.ExpandModal(over_vector=false),
        "Modal 2"   => CUDAExt.ExpandModal(over_vector=true),
    )
end
...
# Correctness check
# ============================================================
function check_correctness(u, a, ud, ad, methods)
...
# ============================================================
function benchmark_single(u, a, ud, ad, methods, S, N)
...
function benchmark_scaling(sizes)
    ...
    method_names = collect(keys(make_methods()))
...
function run_expand_benchmarks()
    ...
    ud_ref = CUDA.cu(u_ref); ad_ref = CUDA.cu(a_ref)
    methods_ref  = make_methods()

    check_correctness(u_ref, a_ref, ud_ref, ad_ref, methods_ref)
    benchmark_single(u_ref, a_ref, ud_ref, ad_ref, methods_ref, S_ref, N_ref)
