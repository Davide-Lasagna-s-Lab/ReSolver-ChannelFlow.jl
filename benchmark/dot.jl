...
function run_benchmarks()
    ...
    twostage_m = CUDAExt.DotTwoStage(ad)
    atomic_m   = CUDAExt.DotAtomic(ad)
    shared_m   = CUDAExt.DotShared(ad)
    ...
    for (M, sz) in zip(Ms, sizes)
        ...
        m_twostage = CUDAExt.DotTwoStage(ad_)
        m_atomic   = CUDAExt.DotAtomic(ad_)
        m_shared   = CUDAExt.DotShared(ad_)
