...
# ============================================================
function make_methods(ad, ud)
    Dict(
        "Loop"      => CUDAExt.ProjectLoop(),
        "Shared"    => CUDAExt.ProjectShared(ad, ud),
    )
end
...
# Correctness check
# ============================================================
function check_correctness(a, u, ad, ud, methods)
...
# ============================================================
function benchmark_single(a, u, ad, ud, methods, S, N)
    println("\n  Detailed benchmark (S=$S, N=$N, T=$(real(eltype(ad)))):")
...
function benchmark_scaling(sizes)
    ...
    ad0 = CUDA.cu(a0); ud0 = CUDA.cu(u0)
    method_names = collect(keys(make_methods(ad0, ud0)))
    ...
    for config in sizes
        ...
        bytes_read    = N * prod(S) * sizeof(eltype(ad))   # u
        bytes_read   += N * prod(S) * sizeof(eltype(ad))   # modes
        bytes_written = prod(size(parent(a))) * sizeof(eltype(ad))
...
function run_project_benchmarks()
    ...
    ad_ref = CUDA.cu(a_ref); ud_ref = CUDA.cu(u_ref)
    methods_ref  = make_methods(ad_ref, ud_ref)

    check_correctness(a_ref, u_ref, ad_ref, ud_ref, methods_ref)
    benchmark_single(a_ref, u_ref, ad_ref, ud_ref, methods_ref, S_ref, N_ref)
